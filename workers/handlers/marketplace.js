...
async function notifyWishlistUsers(listing) {
  var tcg = listing.tcg; var item_name = listing.item_name;
  var set_name = listing.set_name; var price = listing.price;
  var wishlistUsers = [];
  try {
    if (tcg === 'funko') {
...
export async function handleMarketplaceList(request) {
  ...
  var enriched = (result.data || []).map(function(l) {
    var history = l.price_history || []; var price_change = null;
    if (history.length >= 1 && l.price != null) {
      var prev = history[history.length - 1].price;
      if (prev != null && prev !== l.price) price_change = { direction: l.price < prev ? 'down' : 'up', from: prev, to: l.price, pct: Math.round(((l.price - prev) / prev) * 100) };
    }
...
export async function handleMarketplaceCreate(request) {
  ...
  try {
    ...
    var telegram_user_id = body.telegram_user_id; var listing_type = body.listing_type;
    var tcg = body.tcg; var item_name = body.item_name; var contact_telegram = body.contact_telegram;
    if (!telegram_user_id || !listing_type || !tcg || !item_name || !contact_telegram) return jsonResponse({ error: 'Faltan campos obligatorios' }, 400);
    ...
    if (!isPremium) {
      ...
      if ((existRes.data || []).length >= FREE_LISTING_LIMIT) return jsonResponse({ error: 'Los usuarios FREE pueden tener máximo ' + FREE_LISTING_LIMIT + ' anuncios activos.', limit_reached: true }, 403);
    }
    var price = body.price != null ? body.price : null;
    var listing = { telegram_user_id: telegram_user_id, username: body.username || null, listing_type: listing_type, tcg: tcg, item_name: item_name, set_name: body.set_name || null, card_number: body.card_number || null, rarity: body.rarity || null, condition: body.condition || null, variant: body.variant || null, language: body.language || 'es', image_url: body.image_url || null, price: price, currency: body.currency || 'EUR', accepts_trade: body.accepts_trade || false, description: body.description || null, contact_telegram: contact_telegram, original_price: price, price_history: [], status: 'active' };
...
export async function handleMarketplaceUpdate(request) {
  ...
  try {
    var body = await request.json();
    var id = body.id; var telegram_user_id = body.telegram_user_id;
    ...
    if (body.price !== undefined && body.price !== current.price) {
      ...
      updates.price = body.price; updates.price_history = history;
    }
...
export async function handleMarketplaceOffer(request) {
  ...
  try {
    var body = await request.json();
    var listing_id = body.listing_id; var from_user_id = body.from_user_id; var message = body.message;
    ...
    fetch('https://api.telegram.org/bot' + getEnv('TELEGRAM_BOT_TOKEN') + '/sendMessage', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chat_id: listing.telegram_user_id, text: msg, parse_mode: 'Markdown' }) }).catch(function() {});
    return jsonResponse({ success: true });
  } catch(e) { return jsonResponse({ error: e.message }, 500); }
}