import type { es } from './es';

export const en: typeof es = {
  nav: {
    home: 'Home',
    collection: 'Collection',
    explorer: 'Explorer',
    marketplace: 'Market',
    community: 'Community',
    wishlist: 'Wishlist',
    profile: 'Profile',
    ariaPrimary: 'Primary navigation',
  },

  home: {
    greeting: {
      morning: 'Good morning',
      afternoon: 'Good afternoon',
      evening: 'Good evening',
    },
    scanCard: 'Scan a card',
    scanCardDesc: 'Identify and add cards in seconds.',
    openScanner: 'Open scanner',
    yourCollection: 'Your collection',
    dailyTip: 'Daily tip',
    dailyTipDesc: 'Collector tips — from grading advice to market insights.',
    webFallback: 'Open inside Telegram for the full Mini App experience.',
    mainCollection: 'My main collection',
    chooseMain: 'Choose your main collection',
    chooseMainDesc: 'Tap the star on a collection to feature it here.',
    myCollections: 'My collections',
    comingSoon: 'Coming soon',
    goToMarket: 'Go to Marketplace',
    reportBug: 'Report a bug',
  },

  marketplace: {
    title: 'Marketplace',
    subtitle: 'Buy, sell and trade.',
    browse: 'Browse',
    myListings: 'My listings',
    publish: 'Publish',
    sell: 'Selling',
    trade: 'Trading',
    want: 'Wanted',
    newest: 'Newest',
    priceAsc: 'Price ↑',
    priceDesc: 'Price ↓',
    recentChange: 'Recent change',
    priceDrop: 'Price drops',
    priceRise: 'Price rises',
    newlyAdded: 'Newly added',
    allListings: 'All listings',
    noListings: 'No listings match your filters',
    activeListings: 'Active listings',
    goLimit: 'GO for unlimited',
    publishNew: 'Publish new listing',
    noMyListings: 'No listings yet',
    limitReached: 'Limit reached',
    limitDesc: 'FREE users can have {limit} active listings.',
    becomeGo: 'Become GO — 75 Stars',
    whatToDo: 'What do you want to do?',
    category: 'Category',
    itemName: 'Item name *',
    setName: 'Set / Series',
    number: 'Number',
    condition: 'Condition',
    variant: 'Variant',
    language: 'Language',
    imageUrl: 'Image URL (optional)',
    price: 'Price (€)',
    priceOptional: 'Price (€) (optional)',
    maxPrice: 'Max price (€)',
    acceptsTrade: '🔄 Trade',
    description: 'Description',
    contactTelegram: 'Your Telegram username *',
    contactDesc: 'Interested buyers will contact you directly.',
    legalNotice: 'CollectIQ does not intervene in transactions. Illegal items and counterfeits are prohibited.',
    publishing: 'Publishing...',
    sendOffer: 'Send offer',
    sending: 'Sending...',
    offerSent: 'Offer sent',
    offerSentDesc: '@{username} has received your message.',
    contact: 'Contact',
    seller: 'Seller',
    priceHistory: 'Price history',
    current: 'Current',
    reserve: '🤝 Reserve',
    sold: '✅ Sold',
    reactivate: 'Reactivate',
    views: 'views',
    offers: 'offers',
    publishedOn: 'Published on',
    expiresOn: 'Expires on',
    iOffer: 'I offer:',
    notifyMessage: 'The seller will receive a Telegram notification.',
    offerMessage: 'Hi, I\'m interested in your listing...',
    deleteConfirm: 'Delete this listing?',
  },

  bugReport: {
    title: 'Report a bug',
    subtitle: 'Tell us what happened.',
    messagePlaceholder: 'Describe the issue in detail...',
    send: 'Send report',
    sending: 'Sending...',
    sent: '✅ Report sent. Thank you.',
    error: '❌ Failed to send. Please try again.',
  },

  stats: {
    cards: 'Cards',
    unique: 'Unique',
    sets: 'Sets',
    tcgs: 'Sets',
    total: 'Total',
    favorites: 'Favorites',
    estValue: 'Est. value',
  },

  collection: {
    title: 'Collection',
    subtitle: 'Every card you own, in one place.',
    connectTelegram: 'Connect your Telegram account',
    connectTelegramDesc: 'Your collection is tied to your Telegram identity. Open this app inside Telegram.',
    searchPlaceholder: 'Search your collection',
    sort: 'Sort',
    sortRecent: 'Recently added',
    sortName: 'Name A–Z',
    sortAlphabetical: 'Alphabetical',
    sortSet: 'Set',
    sortRarity: 'Rarity',
    sortValue: 'Value',
    noMatches: 'No matches',
    noCardsYet: 'No cards yet',
    noMatchesDesc: 'No cards match "{search}".',
    noCardsYetDesc: 'Add cards from the Explorer to start building your collection.',
    filters: 'Filters',
    clearFilters: 'Clear all filters',
    favorites: 'Favorites',
    allTcgs: 'All',
  },

  variants: {
    title: 'Variant',
    normal: 'Normal',
    normalDesc: 'Standard card with no special finish',
    holofoil: 'Holofoil',
    holofoilDesc: 'The Pokémon image shines with a holographic effect',
    reverseHolofoil: 'Reverse Holofoil',
    reverseHolofoilDesc: 'The border and background shine, but the image is matte',
    firstEdition: 'First Edition',
    firstEditionDesc: 'Has the "1st Edition" stamp — very rare and valuable',
    promo: 'Promo',
    promoDesc: 'Special card from an event, gift or tournament',
    select: 'Select variant',
  },

  cardLanguages: {
    title: 'Card Language',
    select: 'Select your card language',
    en: 'English',
    es: 'Spanish',
    ja: 'Japanese',
    de: 'German',
    fr: 'French',
    it: 'Italian',
    pt: 'Portuguese',
    ko: 'Korean',
    'zh-hant': 'Traditional Chinese',
    th: 'Thai',
    id: 'Indonesian',
    ru: 'Russian',
    pl: 'Polish',
  },

  cardEdit: {
    title: 'Edit card',
    condition: 'Condition',
    conditionNone: 'Not specified',
    mint: 'Mint',
    nearMint: 'Near Mint',
    lightlyPlayed: 'Lightly Played',
    moderatelyPlayed: 'Moderately Played',
    heavilyPlayed: 'Heavily Played',
    damaged: 'Damaged',
    grading: 'Professional grading',
    gradingNone: 'No grading',
    gradingCompany: 'Grading company',
    gradingNoneOption: 'None',
    gradingScore: 'Overall grade (1-10)',
    gradingCertificate: 'Certificate number',
    subGrades: 'Sub-grades',
    centering: 'Centering',
    corners: 'Corners',
    edges: 'Edges',
    surface: 'Surface',
    saveGrading: 'Save grading',
    acquisition: 'Acquisition',
    acquisitionNone: 'Not specified',
    purchasePrice: 'Price paid',
    howObtained: 'How you got it',
    acquiredDate: 'Date acquired',
    saveAcquisition: 'Save acquisition',
    inSleeve: 'In sleeve',
    inBinder: 'In binder',
    personalNotes: 'Personal notes',
    notesPlaceholder: 'Card condition, history, plans...',
    back: '← Back',
  },

  purchaseSources: {
    pack: 'Pack/Booster',
    purchase: 'Purchase',
    trade: 'Trade',
    gift: 'Gift',
    other: 'Other',
  },

  collectionTile: {
    removeFavorite: 'Remove favorite',
    markFavorite: 'Mark as favorite',
    decreaseQty: 'Decrease quantity',
    increaseQty: 'Increase quantity',
    tapConfirm: 'Tap again to confirm',
    removeCard: 'Remove card',
    editCard: 'Edit card',
  },

  explorer: {
    title: 'Explorer',
    subtitle: 'Search the card catalog.',
    searchPlaceholder: 'Card name, number, or set',
    searchAria: 'Search cards',
    resultsFor: 'Results for "{query}"',
    latestCards: 'Latest cards',
    matches: '{count} matches',
    searchFailed: 'Search failed',
    noCardsFound: 'No cards found',
    noCardsFoundDesc: 'No cards match "{query}".',
    startSearching: 'Start searching',
    startSearchingDesc: 'Search by card name, number, or set to browse the catalog.',
    loadMore: 'Load more',
    loadingMore: 'Loading more cards…',
    endOfResults: 'End of results',
    allTcgs: 'All',
    comingSoon: 'soon',
  },

  cardDetails: {
    backToExplorer: 'Back to Explorer',
    couldNotLoad: 'Could not load this card.',
    tryAgain: 'Try again',
    inYourCollection: 'In your collection',
    quantity: 'Quantity: {qty}',
    edit: 'Edit',
    addToCollection: 'Add to My Collection',
    hp: 'HP',
    types: 'Types',
    rarity: 'Rarity',
    set: 'Set',
    artist: 'Artist',
    number: 'Number',
    retreat: 'Retreat',
    evolvesFrom: 'Evolves from',
    legalities: 'Legalities',
    legal: 'Legal',
    banned: 'Banned',
    restricted: 'Restricted',
    reg: 'Reg {mark}',
  },

  scanner: {
    back: 'Back',
    eyebrow: 'Scanner',
    title: 'Scan a card',
    subtitle: 'Point your camera at a card and we\'ll identify it instantly.',
    cameraPreview: 'Camera preview will appear here',
    startScanning: 'Start scanning',
    comingSoon: 'Scanner coming soon',
    aiRecognition: 'AI recognition',
    aiRecognitionDesc: 'The scanner will identify cards across every supported TCG.',
    scanCard: 'Scan card',
    selectOrPhoto: 'Select or photograph a card',
    captureHint: 'For best results, photograph a single card in good lighting without glare.',
    changeImage: 'Change image',
    cancel: 'Cancel',
    analyzeCard: 'Analyze card',
    processingImage: 'Processing image…',
    imageError: 'Could not process the image. Try another photo.',
  },

  wishlist: {
    title: 'Wishlist',
    subtitle: 'Cards you\'re hunting for.',
    searchPlaceholder: 'Search your wishlist',
    empty: 'Your wishlist is empty',
    emptyDesc: 'Add cards from the Explorer.',
  },

  community: {
    title: 'Community',
    subtitle: 'Decks, collections and more.',
    decks: 'Decks',
    collections: 'Collections',
    ranking: 'Ranking',
    comingSoon: 'Community coming soon',
    comingSoonDesc: 'Soon you\'ll be able to see decks, collections and connect with other collectors.',
  },

  decks: {
    title: 'My Decks',
    subtitle: 'Create and share your decks.',
    empty: 'No decks yet',
    emptyDesc: 'Create your first 60-card deck and share it with the community.',
    create: 'Create deck',
  },

  profile: {
    title: 'Profile',
    noUsername: 'No username set',
    telegram: 'Telegram',
    statistics: 'Statistics',
    settings: 'Settings',
    preferences: 'Preferences',
    notifications: 'Notifications',
    language: 'Language',
    privacy: 'Privacy',
    signOut: 'Sign out',
    version: 'CollectIQ · v0.1.0',
    languageSettings: 'Language',
    languageSettingsDesc: 'Choose the interface language.',
    spanish: 'Español',
    english: 'English',
    tools: 'Tools',
    share: 'Share',
    shareCollection: 'Share my collection',
    reportBug: 'Report a bug',
  },

  common: {
    search: 'Search',
    clearSearch: 'Clear search',
    somethingWentWrong: 'Something went wrong',
    tryAgain: 'Try again',
    safeErrorMsg: 'Please try again in a moment.',
    close: 'Close',
    save: 'Save',
    saveChanges: 'Save changes',
    cancel: 'Cancel',
    none: '—',
    yes: 'Yes',
    no: 'No',
    loading: 'Loading...',
    comingSoon: 'Coming soon',
  },

  modal: {
    editCollectionCard: 'Edit collection card',
    addToCollection: 'Add to collection',
    addToCollectionTitle: 'Add to collection',
    editCardTitle: 'Edit card',
    quantity: 'Quantity',
    condition: 'Condition',
    finish: 'Finish',
    language: 'Language',
    edition: 'Edition',
    editionPlaceholder: '1st Edition',
    purchasePrice: 'Purchase price',
    purchasePricePlaceholder: '0.00',
    acquisition: 'Acquisition',
    acquisitionDate: 'Acquisition date',
    notes: 'Notes',
    notesPlaceholder: 'Condition details, purchase story, display plans…',
    favorite: 'Favorite',
    favoriteDesc: 'Pin this card to your favorites',
    showcaseCard: 'Showcase card',
    showcaseDesc: 'Only one card can be your showcase',
  },

  cardImage: {
    imageUnavailable: '{alt} image unavailable',
    noImage: 'No image',
  },

  onboarding: {
    welcome: 'Welcome to CollectIQ!',
    welcomeDesc: 'The ultimate app for TCG card collectors.',
    chooseLanguage: 'Choose your language',
    continue: 'Continue',
  },
};
