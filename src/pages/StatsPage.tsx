  ...
  return (
    <div className="space-y-4 pt-3 pb-24 px-4">
      ...
      <div className="bg-[#111118] border border-white/8 rounded-2xl p-4 space-y-3">
        ...
        {snapshots.length >= 2 && <MiniChart snapshots={snapshots} />}
      ...
      <div className="grid grid-cols-2 gap-2">
        {[
          ...
          { label: 'Cartas unicas', value: uniqueCards, color: 'text-purple-400', icon: '✨' },
          { label: 'Favoritas', value: favorites, color: 'text-yellow-400', icon: '⭐' },
          { label: 'Sets', value: setGroups.length, color: 'text-green-400', icon: '📦' },
        ].map(item => (
            ...
            <div key={group.setName} className="space-y-1">
              ...
              <div className="w-full bg-white/10 rounded-full h-1">
                <div className="bg-blue-500 h-1 rounded-full"
                  style={{ width: (group.value / setGroups[0].value * 100) + '%' }} />
              </div>
            </div>
          ))}
        </div>
              ...
              <div className="space-y-2">
                ...
                <div className={'rounded-xl p-3 text-center ' + (isPositive ? 'bg-green-500/10 border border-green-500/20' : 'bg-red-500/10 border border-red-500/20')}>
                  ...
                  <p className={'text-xs mt-0.5 ' + (isPositive ? 'text-green-400' : 'text-red-400')}>
                    {isPositive ? '+' : ''}{formatPrice(totalMarket - totalPaid)}
                  </p>
                </div>
              </div>
            );
